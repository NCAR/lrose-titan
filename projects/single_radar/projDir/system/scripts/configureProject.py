...
def main():
    ...
    gitProjDir = os.path.join(titanGitDir, 'projects/single_radar/projDir')
    ...
    print(" ")
    print("*********************************************************************")
    print("  configure project - SINGLE RADAR TEST")
    ...
    installLogsDir = os.path.join(installDataDir, 'logs')
    print("Install data dir: ", installDataDir, file=sys.stderr)
    print("Install logs dir: ", installLogsDir, file=sys.stderr)
    cmd = "mkdir -p " + installLogsDir
    runCommand(cmd)

    # copy in data template

    os.chdir(projDir)
    cmd = "rsync -av data_template/* " + installDataDir
    runCommand(cmd)
    ...
    # copy in logs template

    os.chdir(projDir)
    cmd = "mkdir -p rsync -av logs_template/* " + installLogsDir
    runCommand(cmd)
